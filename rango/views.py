...
def visitor_cookie_handler(request):
    #Gets number of visits to the site
    #Using COOKIES.get()
    #If the cookie exists, value returned is casted to an integer
    #If it doesnt exist, the default value 1 is used
    ...
    #If it has been more than a day since the last site visit...
    if (datetime.now() - last_visit_time).days > 0:
        visits = visits + 1
        
        #Update last visit cookie now we have updated the count
...
def index(request):
    ...
    # Return rendered response with cookies to client
    return response
